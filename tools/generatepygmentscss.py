...
NIGHT_MODE_SELECTOR = cssutils.css.Selector(f".{NIGHT_MODE_CLASS} .{PYGMENTS_CLASS}")


def delete_pygments_css_preamble(
    pygments_css_rules: cssutils.css.CSSRuleList, selector: cssutils.css.Selector
):
...
def get_pygments_css(
    style: pygments.style.Style, selector: cssutils.css.Selector
...
def generate_highlighter_pygments_css() -> cssutils.css.CSSStyleSheet:
    ...
    css.cssRules.extend(get_pygments_css(day_style, DAY_MODE_SELECTOR).cssRules)
    css.cssRules.extend(get_pygments_css(night_style, NIGHT_MODE_SELECTOR).cssRules)
    return css
